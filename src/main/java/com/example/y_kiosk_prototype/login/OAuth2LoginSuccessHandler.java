...
public class OAuth2LoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        String providerId = attributes.get("provider").toString();  // 카카오 고유 아이디
        ...
        SocialLogin socialLogin= userService.findSocialLoginByKakaoId(providerId);

        if (socialLogin == null) { // 없으면 신규회원으로 간주
            ...
            //socialLoginRepository.save(socialLogin1);

            userService.addKakaoAccount(newUser, socialLogin1);
            userInfo = newUser;
        } else {
            //Optional<SocialLogin> socialLogin1 = socialLoginRepository.findByKakaoId(providerId);
            //String userId = socialLogin1.get().getUserId();
            //userInfo = userInfoRepository.findByUserId(userId).get();
            String userId = userService.findSocialLoginByKakaoId(providerId).getUserId();
            userInfo = userService.findUserByUserId(userId);
        }
        ...
        ResponseCookie jwtCookie = ResponseCookie.from("accessToken", accessToken)
                ...
                .sameSite("Lax") // CSRF 공경 방어
                .maxAge(60*60) // 한시간 뒤 파기
                .build();

        response.addHeader("Set-Cookie", jwtCookie.toString());

        String targetUrl = "/main"; // 인증이 끝난 후 사용자가 볼 메인 화면.
